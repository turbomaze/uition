...
function Layer(settings) {
    ...
    this.cfg = {
        ...
        patnType: settings.patnType || 'alphanum',
...
function SpatialPooler(cfg) {
    ...
    var s = encoderParams[this.cfg.patnType].n; //len of input in bits
    for (var ai = 0; ai < this.cfg.numCols; ai++) {
        this.columns.push(new Column(
            ai, s, this.cfg.numPotSyn,
            this.cfg.initPerm, this.cfg.permSpread,
            this.cfg.permThresh, this.cfg.minOverlap
        ));
    }
}
SpatialPooler.prototype.process = function(inp) {
    var inpSDR = encode(this.cfg.patnType, inp);
    var sdr = [];
...
SpatialPooler.prototype.getSDR = function(rawInp) {
    var inpSDR = encode(this.cfg.patnType, rawInp);
    var sdr = [];
    var overlaps = this.getOverlaps(inpSDR);
    var thresh = this.getOvlpThreshold(overlaps);
    for (var ai = 0; ai < overlaps.length; ai++) {
