...
var numPotSyn = 48; //with how many bits can SP cols form synapses?
var minOverlap = 4; //# of active synapses for an SP col to be considered
var historyFog = 500; //how far can the alg look into the past?

//temporal learning
var initPerm = 0.20; //initial permanence
var permThresh = 0.20; //determines whether or not a synapse is connected
var permInc = 0.02; //how much permanences are incremented
...
//rendering config
var drawEvery = 0; //how often to draw the TP
...
//pattern[di] = mousePos.slice(0); //for the mouse
        ...
        var pred = TP.getPrediction();
        if (pred === pattern[di]) numRightPred++;
        else numWrongPred++;  
        ...
        var bPct = round(100*numNonBursts/(numBursts+numNonBursts), 2);
        $s('#pred-actv-pct').innerHTML = bPct+'%';

        var iPct = round(100*numRightPred/(numWrongPred+numRightPred), 2);
        $s('#pred-inp-pct').innerHTML = iPct+'%';

        //HTM
        TP.process(
            SP.process(pattern[di]), pattern[di]
        );
...
TemporalPooler.prototype.getPrediction = function() {
    var probs = {};
    for (var ai = 0; ai < alphanum.length; ai++) {
        probs[alphanum.charAt(ai)] = 0;
    }
    for (var ki = 0; ki < numCols; ki++) {
        for (var ci = 0; ci < cellsPerCol; ci++) {
...
SpatialPooler.prototype.process = function(inp) {
    ...
    for (var ai = 0; ai < this.columns.length; ai++) {
        var col = this.columns[ai];
        if (col.ovlpTotal < minActv) col.dopePermanences(permInc);
    }

    return sdr;
};
SpatialPooler.prototype.getSDR = function(rawInp) {
    var inpSDR = encode(this.inpType, rawInp);
    var sdr = [];
    var overlaps = this.getOverlaps(inpSDR);
    var thresh = this.getOvlpThreshold(overlaps);
    for (var ai = 0; ai < overlaps.length; ai++) {
...
function Column(pos, s) { //s is the length of the transformed inputs in bits
    ...
    for (var ai = 0; ai < numPotSyn; ai++) {
        ...
        this.permanences.push(grf(permThresh-0.08, permThresh+0.08));
    }
}
...
Cell.prototype.observe = function(rawInp) {
    ...
    for (var dp in this.lookup) {
        if (this.lookup.hasOwnProperty(dp)) {
            ...
            if (n < historyFog) { //still hasn't cleared the fog
