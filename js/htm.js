...
var historyFog = 20; //500how long ago does the alg look into activity history?
...
var predictionFog = 20; //how far into the past the pred routine considers
...
/* TODO: content editable div to input the alphanum pattern to learn;
    ...
    asyncLoop(
        ...
        function() { //loop finished
            ...
            console.log(
                numRightPred+'/'+(numWrongPred+numRightPred)+' = '+
                round(100*(numRightPred)/(numWrongPred+numRightPred), 2)+
                '% of inputs predicted.'
            );
...
/***********
 * objects */
function Layer(inpType) {
    this.SP = new SpatialPooler(inpType);
...
Cell.prototype.observe = function(rawInp) {
    ...
    for (var dp in this.lookup) {
        if (this.lookup.hasOwnProperty(dp)) {
            ...
            if (n < predictionFog) { //still hasn't cleared the fog
